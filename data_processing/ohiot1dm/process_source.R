...
ICG = function(x, a = 1.1, b = 2){ #1.35
  return( - (x < 80) * ((abs(80 - x))^b) / 30 - (x > 140) * ((abs(x - 140))^a) / 30)
}
# 
HyperIndex = function(x, a = 1.1){ #1.35
...
preprocess1 = function(data0, interval_min = 60,  startdate = NA){
  ...
  data = list(glucose  = data.frame( day = as.numeric(date(dmy_hms(data0$glucose[, 1])) - startdate),  
              ...
              meal     = if( length(which(!is.na(data0$meal[1,]))) ==0){
                                     data.frame(NA)}else{
                                    data.frame(day = as.numeric(date(dmy_hms(data0$meal[, 1])) - startdate),  
                                    daytime = daytime(dmy_hms(data0$meal[, 1])), 
                                    tt = floor(daytime(dmy_hms(data0$meal[, 1])) / interval_min), 
                                    meal = as.numeric(data0$meal[, 3]))
                            },
              exercise   =   if( length(which(!is.na(data0$exercise[1,]))) ==0){
                               data.frame(NA)}else{
                                     data.frame(day = as.numeric(date(dmy_hms(data0$exercise[, 1])) - startdate),  
                                      daytime = daytime(dmy_hms(data0$exercise[, 1])), 
                                      tt = floor(daytime(dmy_hms(data0$exercise[, 1])) / interval_min), 
                                      exercise = as.numeric(data0$exercise[, 2]))
                               },
              heart    =  if( length(which(!is.na(data0$heart[1,]))) ==0){
                            data.frame(NA)}else{
                                data.frame(day = as.numeric(date(dmy_hms(data0$heart[, 1])) - startdate),  
              ...
              airtemp  = if( length(which(!is.na(data0$airtemp[1,]))) ==0){
                               data.frame(NA)}else{ data.frame(day = as.numeric(date(dmy_hms(data0$airtemp[, 1])) - startdate), 
                                    tt = floor(daytime(dmy_hms(data0$airtemp[, 1])) / interval_min), 
                                    airtemp = as.numeric(data0$airtemp[, 2]))
                              },
              steps    = if( length(which(!is.na(data0$steps[1,]))) ==0){
                           data.frame(NA)}else{
                                 data.frame(day = as.numeric(date(dmy_hms(data0$steps[, 1])) - startdate), 
                                    tt = floor(daytime(dmy_hms(data0$steps[, 1])) / interval_min), 
                                    steps = as.numeric(data0$steps[, 2]))
                           },
              acceleration    = if( length(which(!is.na(data0$acceleration[1,]))) ==0){
                                 data.frame(NA)}else{
                              data.frame(day = as.numeric(date(dmy_hms(data0$acceleration[, 1])) - startdate), 
                             tt = floor(daytime(dmy_hms(data0$acceleration[, 1])) / interval_min), 
                             acceleration = as.numeric(data0$acceleration[, 2]))
                }
  )
...
preprocess2_basal = function(data,  interval_min = 60, startbasal = NA){
  ...
  ob = 1
  trate = startbasal
  data1$basal = rbind(data1$basal,  c(lastday + 1, 0, 0)) 
...
preprocess2_aggregate = function(data,  
                                 variables = c("meal", "exercise","bolus",  "glucose", "heart", "gsr", "skintemp", "airtemp", "steps","acceleration"), 
                                 FUN = list(sum, sum, sum, mean, mean, mean, mean, mean, sum,mean), 
                                 glucose_eval = list(ICG, HyperIndex, HypoIndex, M100, InRange), 
                                 eval_names = c("icg", "hyper", "hypo", "m100", "in_range"), 
                                 missing_as_zero = c("meal","exercise", "bolus"), 
                                 interval_min = 60){
  ...
  for (i in 1:length(variables)){
    if(is.na(data[[var_index[i]]][1,1]) ==T){
    na.index = c(na.index,i)
    }
  }
  
  variables = variables[-na.index]
  FUN = FUN[-na.index]
...
preprocess2 = function(data, interval_min = 30, startbasal = NA){
