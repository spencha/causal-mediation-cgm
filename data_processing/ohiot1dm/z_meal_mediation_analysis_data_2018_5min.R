# =============================================================================
# z_meal_mediation_analysis_data_2018_5min.R
# Creates meal-centered windows for 2018 OhioT1DM cohort
#
# Uses data_train/data_test splits from the original OhioT1DM dataset
# (based on study days) to produce:
#   - z_meal_mediation_analysis_data_2018_5min.RData       (full data)
#   - z_meal_mediation_analysis_data_2018_TRAIN_5min.RData (train split)
#   - z_meal_mediation_analysis_data_2018_TEST_5min.RData  (test split)
# =============================================================================
...
# Load 5-minute preprocessed data (contains data_full, data_train, data_test)
load(file.path(DATA_PROCESSING_DIR, "ohiot1dm", "data_preprocessed_2018-5min.RData"))
...
process_subject_list <- function(subject_list, window_id_offset = 0, cohort_label = "2018") {
...
cat("\n=== Processing 2018 OhioT1DM Cohort ===\n")
cat(sprintf("Number of subjects: %d\n", length(data_full)))

# --- Full data ---
cat("\n--- Processing FULL data ---\n")
full_result <- process_subject_list(data_full, window_id_offset = 0, cohort_label = "2018")

# --- Training data ---
cat("\n--- Processing TRAIN data ---\n")
train_result <- process_subject_list(data_train, window_id_offset = 0, cohort_label = "2018")

# --- Test data ---
cat("\n--- Processing TEST data ---\n")
test_result <- process_subject_list(data_test, window_id_offset = 0, cohort_label = "2018")

# =============================================================================
# SAVE RESULTS
# =============================================================================

# Helper to save a result set
save_result <- function(result, filename, extra_vars = list()) {
  # Unpack into the standard variable names expected by downstream code
...
# Save full data
cat("\n=== Saving Results ===\n")
save_result(full_result, "z_meal_mediation_analysis_data_2018_5min.RData")

# Save training data with split_info
train_split_info <- list(
  SPLIT_TYPE = "ohio_original",
  SPLIT_SOURCE = "data_train from data_preprocessed_2018-5min.RData",
...
save_result(train_result, "z_meal_mediation_analysis_data_2018_TRAIN_5min.RData",
            extra_vars = list(split_info = train_split_info))

# Save test data with split_info
test_split_info <- list(
  SPLIT_TYPE = "ohio_original",
  SPLIT_SOURCE = "data_test from data_preprocessed_2018-5min.RData",
  TRAIN_SUBJECTS = unique(train_result$all_cleaned_data$subject_id),
  TEST_SUBJECTS = unique(test_result$all_cleaned_data$subject_id)
)
save_result(test_result, "z_meal_mediation_analysis_data_2018_TEST_5min.RData",
            extra_vars = list(split_info = test_split_info))

# Print summary
cat("\n=== 2018 Meal Window Processing Complete ===\n")
